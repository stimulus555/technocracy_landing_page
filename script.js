...
const bikeSection = document.querySelector("#bikeExperience");
const bikeVideo = document.querySelector("#bikeVideo");
let loadingTimer;
let loadingFinished = false;


// The opening sequence: display the mechanical gears, then reveal the website.
enterButton.addEventListener("click", () => {
  loader.classList.add("active"); enterButton.disabled = true;
  loader.classList.add("active");
  enterButton.disabled = true;
  entryVideo.currentTime = 0;
  entryVideo.play().catch(() => completeLoading());
  let progress = 0;
  ...
  loadingTimer = setInterval(() => {
    progress = Math.min(progress + Math.ceil(Math.random() * 7), 94);
  }, 105);
  entryVideo.addEventListener("ended", completeLoading, { once:true });
});
...
function completeLoading() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => { introScreen.remove(); mainSite.hidden = false; setVideoFrameFromScroll(); }, 700);
  }, 400);
}
...
// Scrub a pre-rendered video instead of playing it normally.
// Scroll at the top shows the assembled engine; lower scroll positions show dismantled frames.
function setVideoFrameFromScroll() {
  if (!Number.isFinite(engineVideo.duration)) return;
...
for (let x=-2.55; x<=2.55; x+=1.7) { const rib=box(.16,3.05,3.65,steel); rib.position.set(x,-.35,0); engine.add(rib); }
  ...
  const heroTravel = hero.offsetHeight * 0.92;
  const progress = Math.min(Math.max((window.scrollY - heroTop) / heroTravel, 0), 1);
  const targetTime = progress * Math.max(engineVideo.duration - 0.04, 0);
...
for (const side of [-1,1]) {
  ...
  // Prevent unnecessary seeks, which keeps the animation smooth while scrolling.
  if (Math.abs(engineVideo.currentTime - targetTime) > 0.025) {
    engineVideo.currentTime = targetTime;
  }
}
...
// A second scroll-scrubbed sequence. The 220vh section gives enough scroll travel
// to play the complete bike video without auto-playing it.
function setBikeFrameFromScroll() {
  if (!Number.isFinite(bikeVideo.duration)) return;
  const start = bikeSection.offsetTop;
  const travel = bikeSection.offsetHeight - window.innerHeight;
  const progress = Math.min(Math.max((window.scrollY - start) / travel, 0), 1);
  // Use only the first two-thirds of the clip. This makes the scroll movement feel slower
  // and avoids rushing through the ending frames.
  const playableDuration = bikeVideo.duration * (2 / 3);
  const targetTime = progress * Math.max(playableDuration - 0.04, 0);
  if (Math.abs(bikeVideo.currentTime - targetTime) > 0.025) bikeVideo.currentTime = targetTime;
}
...
engineVideo.addEventListener("loadedmetadata", () => {
  engineVideo.pause();
  engineVideo.currentTime = 0.01;
  setVideoFrameFromScroll();
});
bikeVideo.addEventListener("loadedmetadata", () => { bikeVideo.pause(); bikeVideo.currentTime = 0.01; setBikeFrameFromScroll(); });
...
render();
window.addEventListener("scroll", () => { setVideoFrameFromScroll(); setBikeFrameFromScroll(); }, { passive:true });
